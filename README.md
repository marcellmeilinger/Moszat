...
### Játékosoknak (Egyszerű letöltés)
Ha csak játszani szeretnél a játékkal, egyszerűen letöltheted a kész, futtatható verziót:
1. Menj a [Releases](../../releases/tag/final_version) oldalra.
2. Töltsd le a **final_version**-höz tartozó `.zip` fájlt.
3. Csomagold ki a letöltött fájlt egy tetszőleges mappába a gépeden.
4. Kattints duplán a `M.A.D.E.exe` fájlra a játék elindításához!

---

### Fejlesztőknek (Forráskód és Fordítás)

Ha szeretnéd módosítani a játékot, vagy a forráskódból fordítani, kövesd az alábbi lépéseket:

#### Futtatás Unity Editorból
1. Klónozd a tárolót:
   ```bash
...
3. A szerkesztő betöltődése után navigálj a `M.A.D.E/Assets/Scenes/` mappába és nyisd meg a `StartMenu` Scene-t.
4. Nyomj a Play (▶️) gombra a játék indításához!

#### Fordítás (Build) önálló alkalmazássá
1. A Unity Editorban válaszd a **File** -> **Build Settings...** menüpontot.
2. Győződj meg róla, hogy a "Scenes In Build" listában szerepelnek a játék pályái megfelelő sorrendben (pl. `StartMenu`, `Level1`, `Level2`, `Level3`). Ha hiányoznak, nyisd meg őket és kattints az "Add Open Scenes" gombra.
3. Válaszd ki a célplatformot a **Platform** listából (pl. Windows).
4. Kattints a **Build** gombra.
5. Válassz vagy hozz létre egy mappát a fordítás kimenetének (pl. egy `Builds/` mappát a projekt gyökerén kívül), és indítsd el a folyamatot.